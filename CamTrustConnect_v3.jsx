...
// ─── STORAGE ──────────────────────────────────────────────────────────────────
const sk = (uid, key) => `user-${uid}-${key}`;
async function dbGet(uid, key) {
  try { const r = await window.storage.get(sk(uid, key)); return r ? JSON.parse(r.value) : null; }
  catch { return null; }
}
async function dbSet(uid, key, value) {
  try { await window.storage.set(sk(uid, key), JSON.stringify(value)); }
  catch (e) { console.error("Save failed", e); }
}
async function getAuthStore() {
  try { const r = await window.storage.get("auth-users"); return r ? JSON.parse(r.value) : {}; }
  catch { return {}; }
}
async function saveAuthStore(store) { await window.storage.set("auth-users", JSON.stringify(store)); }
async function getCurrentUser() {
  try { const r = await window.storage.get("auth-current"); return r ? JSON.parse(r.value) : null; }
  catch { return null; }
}
async function setCurrentUser(user) {
  if (user) await window.storage.set("auth-current", JSON.stringify(user));
  else await window.storage.delete("auth-current");
}
...
const RESOURCES = [
  { title: "IRS EIN Application",                  desc: "Apply for your Employer Identification Number — free and instant online",         link: "https://www.irs.gov/businesses/small-businesses-self-employed/apply-for-an-employer-identification-number-ein-online", category: "Tax"      },
  ...
  { title: "Wave Accounting",                       desc: "Free invoicing, accounting, and receipt scanning built for small businesses",       link: "https://www.waveapps.com",                                                                                              category: "Finance"  },
  ...
  { title: "SCORE Free Mentorship",                 desc: "Free one-on-one mentoring and workshops from experienced business executives",      link: "https://www.score.org",                                                                                                 category: "Support"  },
  ...
  { title: "Gusto Payroll",                         desc: "Simple payroll, benefits, and HR for small businesses and their teams",             link: "https://gusto.com",                                                                                                     category: "Finance"  },
  ...
  { title: "Canva for Business",                    desc: "Design logos, social graphics, presentations, and marketing materials for free",    link: "https://www.canva.com",                                                                                                 category: "Brand"    },
  { title: "Nevada Small Business Development",     desc: "Free advising, training, and resources for Nevada entrepreneurs",                   link: "https://nevadasbdc.org",                                                                                                category: "Support"  },
...
const SERVICE_TYPES = [
  "Business Consultation", "LLC Formation Assistance",
  "Document Preparation", "Notary Services", "HR Compliance Review",
];
      ...
      {ENTITIES.map(e => (
        <div key={e.id} style={{ ...S.card, padding: 0, overflow: "hidden" }}>
          ...
          >
            ...
            <div style={{ fontSize: "18px", color: C.muted, marginLeft: "12px", flexShrink: 0, transform: open === e.id ? "rotate(180deg)" : "none", transition: "transform 0.2s ease" }}>
              v
            </div>
          </div>
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, padding: "20px 22px" }}>
              ...
              <div style={S.grid2}>
                <div style={{ backgroundColor: `${C.success}08`, borderRadius: "10px", padding: "16px", border: `1px solid ${C.success}20` }}>
                  ...
                  {e.pros.map((p, i) => (
                    <div key={i} style={{ fontSize: "13px", color: C.text, marginBottom: "6px", display: "flex", gap: "8px" }}>
                ...
                <div style={{ backgroundColor: `${C.error}06`, borderRadius: "10px", padding: "16px", border: `1px solid ${C.error}15` }}>
                  ...
                  {e.cons.map((c, i) => (
                    <div key={i} style={{ fontSize: "13px", color: C.text, marginBottom: "6px", display: "flex", gap: "8px" }}>
                      <span style={{ color: C.error, flexShrink: 0 }}>-</span>{c}
                    </div>
                  ))}
                </div>
              </div>
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "18px" }}>
        {categories.map(cat => (
          <button
            key={cat}
            style={{ ...S.btn("sm"), backgroundColor: filter === cat ? C.maroon : "transparent", color: filter === cat ? "#fff" : C.muted, border: `1px solid ${filter === cat ? C.maroon : C.border}` }}
...
export default function CamTrustConnect() {
  ...
  const loadUserData = async (u) => {
    setUser(u);
    setIsGuest(false);
    const ch = await dbGet(u.uid, "checklist") || {};
    const vf = await dbGet(u.uid, "vault") || [
    ...
    const ap = await dbGet(u.uid, "appointments") || [];
    setChecked(ch); setVaultFiles(vf); setAppointments(ap);
  ...
  const toggleCheck = async (id) => {
    ...
    setChecked(next);
    await dbSet(user.uid, "checklist", next);
    flashSave("Progress saved");
  };

  const addVaultFile = async () => {
    ...
    setVaultFiles(next);
    await dbSet(user.uid, "vault", next);
    setUploadName(""); flashSave("Document added");
  };

  const removeVaultFile = async (idx) => {
    const next = vaultFiles.filter((_, i) => i !== idx);
    setVaultFiles(next);
    await dbSet(user.uid, "vault", next);
    flashSave("Document removed");
  };
  ...
  const handleBook = async () => {
    ...
    await dbSet(user.uid, "appointments", next);
    setBooked(true);
  };
  ...
  const handleGuest = () => { setIsGuest(true); setActivePage("home"); setAuthLoading(false); };
  ...
  if (authLoading) return (
    <div style={S.fullCenter}><div style={{ color: C.muted, fontSize: "14px" }}>Loading...</div></div>
  );

  if (!user && !isGuest) return <AuthScreen onLogin={loadUserData} onGuest={handleGuest} />;

  // Guest trying to access auth-only pages
  ...
  return (
    <div style={S.app}>
        ...
        <div style={S.userBar}>
          {user ? (
            <>
              ...
              <button style={{ ...S.btn("ghost"), marginTop: "10px", padding: "5px 0", fontSize: "12px", color: C.muted }} onClick={handleLogout}>
                Sign out
              </button>
            </>
          ) : (
            <>
              ...
              <button style={{ ...S.btn("outline"), fontSize: "12px", padding: "7px 12px" }} onClick={() => { setIsGuest(false); setActivePage("home"); }}>
                Sign in
              </button>
            </>
          )}
      ...
      <div style={S.main}>
        {saveMsg && (
          <div style={{ position: "fixed", top: "16px", right: "24px", backgroundColor: C.surface, border: `1px solid ${C.border}`, borderRadius: "8px", padding: "8px 14px", fontSize: "13px", color: C.success, zIndex: 100 }}>
            + {saveMsg}
          </div>
        )}

        {/* LOCKED — guest trying to access auth page */}
        {pageToShow === "locked" && (
          <div>
            <div style={S.pageHeader}>
            ...
            <div style={{ ...S.card, textAlign: "center", padding: "48px 24px" }}>
              ...
              <div style={{ fontSize: "14px", color: C.muted, marginBottom: "24px", lineHeight: "1.6" }}>
                It's free and takes less than a minute. Your progress will be saved every time you come back.
              </div>
              <button style={S.btn()} onClick={() => { setIsGuest(false); setActivePage("home"); }}>
              ...
              <>
                <div style={S.grid2}>
                  <div style={S.statCard}>
                    <div style={S.statNumber}>{completedCount}/{CHECKLIST.length}</div>
                    <div style={S.statLabel}>Checklist steps done</div>
                  </div>
                  <div style={S.statCard}>
                    <div style={S.statNumber}>{vaultFiles.length}</div>
                    <div style={S.statLabel}>Documents stored</div>
                  </div>
                </div>
            ...
            {!user && (
              <div style={{ ...S.card, backgroundColor: `${C.maroon}06`, border: `1px solid ${C.maroon}20`, marginBottom: "14px" }}>
                <div style={{ fontSize: "14px", color: C.text, lineHeight: "1.7" }}>
                  CamTrust Consulting Group supports entrepreneurs, notary clients, and small business owners with the tools and guidance to move with confidence. Create a free account to track your progress, save documents, and book appointments.
                </div>
                <button style={{ ...S.btn(), marginTop: "14px" }} onClick={() => { setIsGuest(false); setActivePage("home"); }}>
                  Create a free account
                </button>
              </div>
            )}
            ...
            <div style={S.card}>
              ...
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {navItems.filter(n => n.id !== "home").map(item => (
                  <button key={item.id} style={S.btn("outline")} onClick={() => setActivePage(item.id)}>
                    {item.label}
                  </button>
                ))}
                {isGuest && (
                  <>
                    <button style={S.btn("outline")} onClick={() => setActivePage("checklist")}>Startup Checklist</button>
                    <button style={S.btn("outline")} onClick={() => setActivePage("vault")}>Document Vault</button>
                    <button style={S.btn("outline")} onClick={() => setActivePage("appointments")}>Book Appointment</button>
                  </>
                )}
              </div>
            </div>
            ...
            <div style={S.card}>
              ...
              {vaultFiles.length === 0 ? (
                <div style={S.emptyState}>
                  <div style={{ fontSize: "14px" }}>No documents yet. Add one above.</div>
                </div>
              ) : (
                vaultFiles.map((f, i) => (
          ...
          <div>
            ...
            {booked ? (
              ...
              <>
                ...
                <button
                  style={{ ...S.btn(), opacity: (clientName && clientEmail && selDate && selTime && selService) ? 1 : 0.4, cursor: (clientName && clientEmail && selDate && selTime && selService) ? "pointer" : "not-allowed" }}
                  onClick={handleBook}
                >
                  Confirm Appointment
                </button>
